 ...
 * @constructor
 */
export async function Install (command: string = "npm", args: ( string | string )[] = [ "install", PKG, "--verbose" ], cwd: string = PKG, ignore: boolean = false): Promise<boolean> {
    const instance = Install.name;

    process.stdout.write( ANSI.white ( "[" + instance + "]" ) + " >>> " + "Installing Pre-Build Dependencies ..." + "\n" );
    ...
    return new Promise( (resolve, reject) => {
        ...
        subprocess.stderr?.on( "data", (chunk: Buffer | string | string[]) => {
            if (!ignore) {
                ...
                chunk.forEach( (line) => {
                    const raw = line.replaceAll( Control, "" ).replace( "[0m", "" ).trim();
                    ( raw !== "" && raw.includes("cache miss") ) && process.stdout.write( ANSI.white( "[" + instance + "]" ) + " >>> " + line.trim() + "\n" );
                } );
            }
        } );
...
Reflect.set(Install, "name", "Install");
